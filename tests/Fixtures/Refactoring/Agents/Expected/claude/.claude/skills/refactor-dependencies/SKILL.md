...
Analyze the class supplied with this invocation. Call the `refactoring_dependencies` MCP tool when the Agent Kit MCP server is available; otherwise run `php artisan agent-kit:refactor-dependencies "<target>" --json`.
